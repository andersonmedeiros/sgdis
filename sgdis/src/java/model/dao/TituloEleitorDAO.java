...
public class TituloEleitorDAO {
    ...
    private final String GETTITULOELEITORBYID = "SELECT * " +
                                                "FROM Endereco " + 
                                                "WHERE id = ?";
       
    public TituloEleitor getTituloEleitorById(int idTitEleitor){
        TituloEleitor titEleitor = new TituloEleitor();
        CidadeDAO cidDAO = new CidadeDAO();
        AlunoDAO alDAO = new AlunoDAO();
        try {
            conn = ConnectionFactory.getConnection();
            pstm = conn.prepareStatement(GETTITULOELEITORBYID);
            pstm.setInt(1, idTitEleitor);
